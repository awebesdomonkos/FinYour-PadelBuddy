...
import { currentLang, localeFor } from '../hooks/useI18n.ts';
...
export default function ChatDrawer({
  ...
  const dateStr = gameDateTime ? new Date(gameDateTime).toLocaleDateString(localeFor(currentLang()), { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : '';

  const safeTimestamp = (ts: string | undefined) => {
    if (!ts) return '';
    const d = new Date(ts);
    return isNaN(d.getTime()) ? '' : d.toLocaleTimeString(localeFor(currentLang()), { hour: '2-digit', minute: '2-digit' });
  };
  ...
  const quickMessages = t('chat.quick') as unknown as string[];
  ...
  return (
    <motion.div
      ...
      {...dialogProps}
      aria-label={game.location || t('chat.fallbackTitle')}
      className="fixed inset-y-0 right-0 w-full max-w-sm bg-[#F8F8F5] z-[60] outline-none shadow-2xl border-l border-[#141414]/10 flex flex-col"
    >
      {/* Header */}
      <div className="p-4 pt-[calc(1rem+env(safe-area-inset-top,0px))] bg-[#141414] text-white">
        <div className="flex items-center gap-3 mb-3">
          <button onClick={onClose} aria-label={t('a11y.back')} className="p-2.5 hover:bg-white/10 rounded-xl transition-colors">
            <ArrowLeft className="w-5 h-5" aria-hidden="true" />
          </button>
          <div className="flex-1 min-w-0">
            <h3 className="font-black uppercase tracking-tight leading-none truncate">{game.location || t('chat.fallbackTitle')}</h3>
        ...
        <div className="flex gap-1">
          {[
            ...
            { key: 'members', label: t('chat.members'), count: joinedUsers.length },
            ...(isOwner && pendingRequests.length > 0 ? [{ key: 'requests', label: t('chat.requests'), count: pendingRequests.length }] : [])
          ].map(tab => (
            <button
              key={tab.key}
          ...
          <div className="p-4 space-y-2">
            ...
            {joinedUsers.map(user => (
              <div key={user.id} className="bg-white p-3 rounded-2xl border border-[#141414]/5 flex items-center gap-3">
                ...
                {user.id === game.creatorId && (
                  <span className="px-2 py-0.5 bg-[#E2FF3B] text-[#141414] text-[8px] font-black uppercase rounded">{t('chat.host')}</span>
                )}
              </div>
            ))}
            {/* Empty slots */}
            {Array.from({ length: Math.max(0, Number(game.requiredPlayers || 4) - joinedUsers.length) }).map((_, i) => (
              <div key={i} className="bg-[#141414]/5 border border-dashed border-[#141414]/10 rounded-2xl p-3 flex items-center gap-3 opacity-40">
                ...
                <span className="text-xs font-bold uppercase tracking-widest">{t('chat.freeSlot')}</span>
              </div>
            ))}
          </div>
        )}
          ...
          <div className="p-4 space-y-2">
            {pendingRequests.length === 0 ? (
              <div className="py-16 text-center opacity-30">
                ...
                <p className="text-xs font-black uppercase tracking-widest">{t('chat.noPending')}</p>
              </div>
            ) : (
                ...
                <div key={req.userId} className="bg-white p-4 rounded-2xl border border-[#141414]/5 shadow-sm">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-[10px] opacity-40 font-bold uppercase">{t('chat.wantsToJoin')}</p>
                    </div>
                    <div className="flex gap-2">
                      <button
                        onClick={() => onApprove(req.userId, false)}
                        aria-label={`${t('chat.reject')}: ${req.userName}`}
                      ...
                      <button
                        ...
                        aria-label={`${t('chat.approve')}: ${req.userName}`}
                        className="w-11 h-11 bg-green-50 text-green-500 rounded-xl flex items-center justify-center hover:bg-green-100 transition-colors"
                      >
