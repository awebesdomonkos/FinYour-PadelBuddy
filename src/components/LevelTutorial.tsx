  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-[#141414]/90 backdrop-blur-sm">
      <motion.div
        ...
        className="bg-[#1A2233] w-full max-w-md rounded-[32px] overflow-hidden shadow-2xl relative"
      >
        <button
          onClick={onClose}
          className="absolute top-6 right-6 p-2 bg-[#FFFFFF]/5 rounded-xl hover:bg-[#FFFFFF]/8 transition-colors z-20"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="p-8">
          <div className="flex items-center gap-4 mb-8">
            <div className="w-12 h-12 bg-[#00E676] rounded-2xl flex items-center justify-center">
          ...
          <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2 scrollbar-none">
            {Object.values(SkillLevel).map(lvl => (
              <div key={lvl} className="p-4 bg-[#FFFFFF]/5 rounded-2xl border border-transparent hover:border-[#00E676]/40/50 transition-all group">
          ...
          <button
            onClick={onClose}
            className="w-full mt-8 bg-[#141414] text-white py-4 rounded-2xl font-black uppercase tracking-widest hover:bg-[#00C853] transition-colors"
          >
            {t('common.gotIt')}
          </button>
        </div>
